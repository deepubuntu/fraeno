...
async function sendConfirmation(env, message) {
  ...
  // Do not retry ambiguous failures, which could send duplicate confirmations.
  if (response.status !== 201) {
    await response.body?.cancel();
    throw new Error(`Brevo confirmation failed (HTTP ${response.status})`);
  }
  await response.body?.cancel();
}
